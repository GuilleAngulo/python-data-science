...
Original file is located at
    https://colab.research.google.com/drive/1SeDy1oNLM_E6m2NVMFCCpEL-duVLEq-a
...
import pandas as pd
imdb = pd.read_csv("https://raw.githubusercontent.com/GuilleAngulo/python-data-science/master/data/movie_metadata_imdb.csv")
imdb.head()
...
imdb_usa['imdb_score'] = imdb_usa['gross'] - imdb_usa['budget']
...
media_imdb = imdb_usa.query("imdb_score >= 0")[["imdb_score", "color"]].dropna()
color_or_bw = imdb.query("color in ['Color', ' Black and White']").dropna()

fig, axes = plt.subplots(ncols=2,figsize=(20,8))

plt.figure(figsize=(16, 8))

sns.scatterplot(x="color", y="imdb_score", data = media_imdb, ax = axes[0])

sns.boxplot(data = color_or_bw, x = "color", y = "imdb_score", ax = axes[1])

filmes_color = color_or_bw.query("color == 'Color'")
filmes_bw = color_or_bw.query("color == ' Black and White'")

desc_color = filmes_color["imdb_score"].describe()
desc_bw = filmes_bw["imdb_score"].describe()
...
filme_procurado = budget_gross.query("budget > 250000000 & lucro < -150000000")
id_procurado = filme_procurado.index
filme_prejuizo = imdb_usa.query("index == @id_procurado")
filme_prejuizo["movie_title"]
...
budget_by_year = imdb_usa.query("title_year > 2000").groupby("title_year").mean()["budget"]
lucro_by_year = imdb_usa.query("title_year > 2000").groupby("title_year").mean()["lucro"]

fig, axes = plt.subplots(ncols=2,figsize=(12,6))
budget_by_year.plot(ax = axes[0], title="Budget by year", x="Years", y="Budget") 
lucro_by_year.plot(ax = axes[1], title="Benefit by year")
...
filmes_procurados = budget_gross_per_year.query("lucro > 0 & title_year <= 1940")
array_filmes_procurados = filmes_procurados.index.array
filmes_posguerra_lucros = imdb_usa.query("index in @array_filmes_procurados")
filmes_posguerra_lucros.sort_values("lucro", ascending=False)["movie_title"]
...
directores_procurados = gross_director.query("filmes_irmaos > 15 & filmes_irmaos < 20 & dindin < 100000000").dropna()
directores_procurados["director_name"].unique()
...
imdb_usa_2000[["gross", "budget", "lucro", "title_year"]].corr()
...
budget_gross = imdb_usa.query("budget >0 | gross > 0")[["gross", "lucro"]].dropna()

sns.lmplot(x="gross", y="lucro", data = budget_gross)
